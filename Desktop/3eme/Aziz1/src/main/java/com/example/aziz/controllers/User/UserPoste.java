    ...
    private void loadPosts(int choix) {
        ...
        try {
            if (choix == 1) {
                ...
                postList = postService.getByIdUser(1);
            }
...
/**************** 4 ***************************/
        for (Post post : postList) {
            if (post.isVisible()) { // Check if the post is visible
...
ScrollePan.setContent(reclamationFlowPane); */
        ...
        private VBox createPostVBox (Post post,int choix){
            ...
            if (c != null) {
                commentLabel.setText("Lasted comment at " + String.valueOf(c.getCreatedatcomment()));
                commentDataLabel.setText("Comment data : " + c.getContentcomment());
            }
...
/***********************************5********************** */
            ImageView visibilityImageView = new ImageView(new Image(getClass().getResourceAsStream("/image/visibility.png")));
            visibilityImageView.setFitHeight(35.0);
            visibilityImageView.setFitWidth(42.0);
            visibilityImageView.setLayoutX(470.0);//Ajjted layout position to avoid overlapping with delete and edit icons
            visibilityImageView.setLayoutY(10.0);
            visibilityImageView.setPickOnBounds(true);
            visibilityImageView.setPreserveRatio(true);

            visibilityImageView.setOnMouseClicked(event -> {

                try {
                    // Toggle visibility of the post
                    boolean isVisible = !post.isVisible();

                    // Update visibility in the database
                    postService.updateVisibility(post.getId(), isVisible);

                    // Reload posts to reflect the changes
                    loadPosts(choix);

                    // Show success message or handle accordingly
                    showSuccessAlert("Post visibility updated successfully");
                } catch (SQLException e) {
                    // Handle any SQL exception
                    showErrorAlert("Error updating post visibility: " + e.getMessage());
                }
            });
/***********************************5***********************/
            ...
            if (choix == 1) {
                pane.getChildren().addAll(mediaView, imageView, jouerbtn, pausebtn, arreterbtn, videoBtn, imageBtn, userLabel,
                        dateLabel, contentLabel, commentLabel, commentDataLabel, commentTextArea, addBtn, visibilityImageView, CheckComment);
            } else {
                pane.getChildren().addAll(mediaView, imageView, jouerbtn, pausebtn, arreterbtn, videoBtn, imageBtn, userLabel,
                        dateLabel, contentLabel, commentLabel, commentDataLabel, commentTextArea, addBtn, visibilityImageView, editImageView, deleteImageView, CheckComment);
            }
